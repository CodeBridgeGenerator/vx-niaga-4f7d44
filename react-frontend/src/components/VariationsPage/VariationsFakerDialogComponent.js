...
import variationsFakerFactory from "./variationsFakerFactory";
...
const VariationsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = variationsFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("variations").create(elem));
...
export default connect(mapState, mapDispatch)(VariationsFakerDialogComponent);
