...
import purchaseProductFakerFactory from "./purchaseProductFakerFactory";
...
const PurchaseProductFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = purchaseProductFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("purchaseProduct").create(elem));
...
export default connect(mapState, mapDispatch)(PurchaseProductFakerDialogComponent);
