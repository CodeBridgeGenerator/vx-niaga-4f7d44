...
import seederData from "./purchaseProduct.seeder.json";
...
const PurchaseProductSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("purchaseProduct").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(mapState, mapDispatch)(PurchaseProductSeederDialogComponent);
