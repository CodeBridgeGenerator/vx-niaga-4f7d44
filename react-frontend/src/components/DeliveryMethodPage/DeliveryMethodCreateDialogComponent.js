...
const DeliveryMethodCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            branchId: _entity?.branchId,name: _entity?.name,createdAt: _entity?.createdAt,updatedAt: _entity?.updatedAt,
        ...
        const result = await client.service("deliveryMethod").create(_data);
        props.onHide();
        props.alert({ type: "success", title: "Create info", message: "Info Delivery Method created successfully" });
        props.onCreateResult(result);
        } catch (error) {
            console.log("error", error);
            setError(getSchemaValidationErrorsStrings(error) || "Failed to create");
            props.alert({ type: "error", title: "Create", message: "Failed to create in Delivery Method" });
        }
        setLoading(false);
    };
    ...
    return (
        <Dialog header="Create Delivery Method" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max" footer={renderFooter()} resizable={false}>
            <div className="grid p-fluid overflow-y-auto"
            style={{ maxWidth: "55vw" }} role="deliveryMethod-create-dialog-component">
...
export default connect(mapState, mapDispatch)(DeliveryMethodCreateDialogComponent);
