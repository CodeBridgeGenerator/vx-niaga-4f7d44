...
import deliveryMethodFakerFactory from "./deliveryMethodFakerFactory";
...
const DeliveryMethodFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = deliveryMethodFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("deliveryMethod").create(elem));
...
export default connect(mapState, mapDispatch)(DeliveryMethodFakerDialogComponent);
