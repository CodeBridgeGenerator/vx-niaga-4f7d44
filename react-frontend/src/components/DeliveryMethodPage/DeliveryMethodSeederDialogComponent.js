...
import seederData from "./deliveryMethod.seeder.json";
...
const DeliveryMethodSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("deliveryMethod").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(mapState, mapDispatch)(DeliveryMethodSeederDialogComponent);
