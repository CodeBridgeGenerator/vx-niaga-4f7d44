...
const InvoiceProductsCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            invoiceId: _entity?.invoiceId,productId: _entity?.productId,variationId: _entity?.variationId,quantity: _entity?.quantity,price: _entity?.price,tax: _entity?.tax,discount: _entity?.discount,description: _entity?.description,createdAt: _entity?.createdAt,updatedAt: _entity?.updatedAt,
        ...
        props.onHide();
        props.alert({ type: "success", title: "Create info", message: "Info Invoice Products created successfully" });
