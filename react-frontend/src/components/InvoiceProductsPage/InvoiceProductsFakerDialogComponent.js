...
import invoiceProductsFakerFactory from "./invoiceProductsFakerFactory";
...
const InvoiceProductsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = invoiceProductsFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("invoiceProducts").create(elem));
...
export default connect(mapState, mapDispatch)(InvoiceProductsFakerDialogComponent);
