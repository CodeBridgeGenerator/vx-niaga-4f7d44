...
import seederData from "./invoiceProducts.seeder.json";
...
const InvoiceProductsSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("invoiceProducts").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(mapState, mapDispatch)(InvoiceProductsSeederDialogComponent);
