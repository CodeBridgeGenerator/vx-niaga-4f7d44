...
const InvoiceProductsDataTable = ({ items, fields, onEditRow, onRowDelete, onRowClick, searchDialog, setSearchDialog,   showUpload, setShowUpload,
...
const p_numberTemplate5 = (rowData, { rowIndex }) => <p >{rowData.tax}</p>
const p_numberTemplate6 = (rowData, { rowIndex }) => <p >{rowData.discount}</p>
...
const p_numberTemplate9 = (rowData, { rowIndex }) => <p >{rowData.updatedAt}</p>
    ...
    const paginatorRight = DownloadCSV({ data : items, fileName : "invoiceProducts"});
...
<Column field="invoiceId" header="invoice_id" body={p_numberTemplate0} filter={selectedFilterFields.includes("invoiceId")} hidden={selectedHideFields?.includes("invoiceId")}  sortable style={{ minWidth: "8rem" }} />
...
<Column field="tax" header="tax" body={p_numberTemplate5} filter={selectedFilterFields.includes("tax")} hidden={selectedHideFields?.includes("tax")}  sortable style={{ minWidth: "8rem" }} />
<Column field="discount" header="discount" body={p_numberTemplate6} filter={selectedFilterFields.includes("discount")} hidden={selectedHideFields?.includes("discount")}  sortable style={{ minWidth: "8rem" }} />
...
<Column field="updatedAt" header="updated_at" body={p_numberTemplate9} filter={selectedFilterFields.includes("updatedAt")} hidden={selectedHideFields?.includes("updatedAt")}  sortable style={{ minWidth: "8rem" }} />
        ...
        <Dialog header="Upload InvoiceProducts Data" visible={showUpload} onHide={() => setShowUpload(false)}>
        <UploadService 
          user={user} 
          serviceName="invoiceProducts"            
          onUploadComplete={() => {
            setShowUpload(false); // Close the dialog after upload
          }}/>
      </Dialog>

      <Dialog header="Search InvoiceProducts" visible={searchDialog} onHide={() => setSearchDialog(false)}>
      Search
    </Dialog>
    <Dialog
        header="Filter Users"
