...
import seederData from "./deliveryDetail.seeder.json";
...
const DeliveryDetailSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("deliveryDetail").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(mapState, mapDispatch)(DeliveryDetailSeederDialogComponent);
