...
const DeliveryDetailCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            deliveryId: _entity?.deliveryId,productId: _entity?.productId,variationId: _entity?.variationId,deliveredQty: _entity?.deliveredQty,createdAt: _entity?.createdAt,updatedAt: _entity?.updatedAt,
        ...
        props.onHide();
        props.alert({ type: "success", title: "Create info", message: "Info Delivery Detail created successfully" });
