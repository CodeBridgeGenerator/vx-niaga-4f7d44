...
import deliveryDetailFakerFactory from "./deliveryDetailFakerFactory";
...
const DeliveryDetailFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = deliveryDetailFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("deliveryDetail").create(elem));
...
export default connect(mapState, mapDispatch)(DeliveryDetailFakerDialogComponent);
