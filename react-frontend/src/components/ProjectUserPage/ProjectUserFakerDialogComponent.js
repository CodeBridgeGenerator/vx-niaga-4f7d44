...
import projectUserFakerFactory from "./projectUserFakerFactory";
...
const ProjectUserFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = projectUserFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("projectUser").create(elem));
...
export default connect(mapState, mapDispatch)(ProjectUserFakerDialogComponent);
