...
import seederData from "./invoiceType.seeder.json";
...
const InvoiceTypeSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("invoiceType").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(mapState, mapDispatch)(InvoiceTypeSeederDialogComponent);
