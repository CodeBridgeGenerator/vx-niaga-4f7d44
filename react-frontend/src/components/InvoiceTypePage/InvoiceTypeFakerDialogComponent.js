...
import invoiceTypeFakerFactory from "./invoiceTypeFakerFactory";
...
const InvoiceTypeFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = invoiceTypeFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("invoiceType").create(elem));
...
export default connect(mapState, mapDispatch)(InvoiceTypeFakerDialogComponent);
