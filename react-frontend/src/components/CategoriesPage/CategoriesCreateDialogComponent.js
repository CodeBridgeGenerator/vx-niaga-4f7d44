...
const CategoriesCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            branchId: _entity?.branchId,name: _entity?.name,type: _entity?.type,image: _entity?.image,createdAt: _entity?.createdAt,updatedAt: _entity?.updatedAt,
        ...
        const result = await client.service("categories").create(_data);
        props.onHide();
        props.alert({ type: "success", title: "Create info", message: "Info Categories created successfully" });
        props.onCreateResult(result);
        } catch (error) {
            console.log("error", error);
            setError(getSchemaValidationErrorsStrings(error) || "Failed to create");
            props.alert({ type: "error", title: "Create", message: "Failed to create in Categories" });
        }
        setLoading(false);
    };
    ...
    return (
        <Dialog header="Create Categories" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max" footer={renderFooter()} resizable={false}>
            <div className="grid p-fluid overflow-y-auto"
            style={{ maxWidth: "55vw" }} role="categories-create-dialog-component">
...
<div className="col-12 md:col-6 field mt-5">
            <span className="align-items-center">
                ...
                <InputNumber id="type" className="w-full mb-3 p-inputtext-sm" value={_entity?.type} onChange={(e) => setValByKey("type", e.value)}  />
            </span>
            <small className="p-error">
...
export default connect(mapState, mapDispatch)(CategoriesCreateDialogComponent);
