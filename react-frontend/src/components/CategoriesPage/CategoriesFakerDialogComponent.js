...
import categoriesFakerFactory from "./categoriesFakerFactory";
...
const CategoriesFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = categoriesFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("categories").create(elem));
...
export default connect(mapState, mapDispatch)(CategoriesFakerDialogComponent);
