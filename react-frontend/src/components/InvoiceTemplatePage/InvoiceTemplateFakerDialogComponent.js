...
import invoiceTemplateFakerFactory from "./invoiceTemplateFakerFactory";
...
const InvoiceTemplateFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = invoiceTemplateFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("invoiceTemplate").create(elem));
...
export default connect(mapState, mapDispatch)(InvoiceTemplateFakerDialogComponent);
