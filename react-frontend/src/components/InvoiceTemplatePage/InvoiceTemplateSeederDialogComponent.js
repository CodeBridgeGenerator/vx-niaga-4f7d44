...
import seederData from "./invoiceTemplate.seeder.json";
...
const InvoiceTemplateSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("invoiceTemplate").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(mapState, mapDispatch)(InvoiceTemplateSeederDialogComponent);
