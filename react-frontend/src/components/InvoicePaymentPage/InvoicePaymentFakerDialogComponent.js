...
import invoicePaymentFakerFactory from "./invoicePaymentFakerFactory";
...
const InvoicePaymentFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = invoicePaymentFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("invoicePayment").create(elem));
...
export default connect(mapState, mapDispatch)(InvoicePaymentFakerDialogComponent);
