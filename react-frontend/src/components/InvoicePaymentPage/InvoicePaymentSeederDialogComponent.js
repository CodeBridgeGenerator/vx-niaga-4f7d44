...
import seederData from "./invoicePayment.seeder.json";
...
const InvoicePaymentSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("invoicePayment").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(mapState, mapDispatch)(InvoicePaymentSeederDialogComponent);
