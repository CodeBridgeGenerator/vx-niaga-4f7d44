...
import purchaseFakerFactory from "./purchaseFakerFactory";
...
const PurchaseFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = purchaseFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("purchase").create(elem));
...
export default connect(mapState, mapDispatch)(PurchaseFakerDialogComponent);
