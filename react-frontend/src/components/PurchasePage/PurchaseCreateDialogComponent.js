...
const PurchaseCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            branchId: _entity?.branchId,vendorId: _entity?.vendorId,warehouseId: _entity?.warehouseId,purchaseType: _entity?.purchaseType,date: _entity?.date,status: _entity?.status,categoryId: _entity?.categoryId,createdBy: _entity?.createdBy,createdAt: _entity?.createdAt,updatedAt: _entity?.updatedAt,
        ...
        const result = await client.service("purchase").create(_data);
        props.onHide();
        props.alert({ type: "success", title: "Create info", message: "Info Purchase created successfully" });
        props.onCreateResult(result);
        } catch (error) {
            console.log("error", error);
            setError(getSchemaValidationErrorsStrings(error) || "Failed to create");
            props.alert({ type: "error", title: "Create", message: "Failed to create in Purchase" });
        }
        setLoading(false);
    };
    ...
    return (
        <Dialog header="Create Purchase" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max" footer={renderFooter()} resizable={false}>
            <div className="grid p-fluid overflow-y-auto"
            style={{ maxWidth: "55vw" }} role="purchase-create-dialog-component">
