...
const NewsCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            title: _entity?.title,startDate: _entity?.startDate,endDate: _entity?.endDate,description: _entity?.description,branchId: _entity?.branchId,createdAt: _entity?.createdAt,updatedAt: _entity?.updatedAt,
        ...
        props.onHide();
        props.alert({ type: "success", title: "Create info", message: "Info News created successfully" });
        props.onCreateResult(result);
        } catch (error) {
            console.log("error", error);
            setError(getSchemaValidationErrorsStrings(error) || "Failed to create");
            props.alert({ type: "error", title: "Create", message: "Failed to create in News" });
        }
        setLoading(false);
    };
    ...
    return (
        <Dialog header="Create News" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max" footer={renderFooter()} resizable={false}>
            <div className="grid p-fluid overflow-y-auto"
            style={{ maxWidth: "55vw" }} role="news-create-dialog-component">
...
export default connect(mapState, mapDispatch)(NewsCreateDialogComponent);
