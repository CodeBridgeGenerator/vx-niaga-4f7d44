...
const TransactionFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = transactionFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("transaction").create(elem));
...
export default connect(mapState, mapDispatch)(TransactionFakerDialogComponent);
