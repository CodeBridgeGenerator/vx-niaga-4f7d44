...
import projectClientFakerFactory from "./projectClientFakerFactory";
...
const ProjectClientFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = projectClientFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("projectClient").create(elem));
...
export default connect(mapState, mapDispatch)(ProjectClientFakerDialogComponent);
