...
import vendorAddressFakerFactory from "./vendorAddressFakerFactory";
...
const VendorAddressFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = vendorAddressFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("vendorAddress").create(elem));
...
export default connect(mapState, mapDispatch)(VendorAddressFakerDialogComponent);
