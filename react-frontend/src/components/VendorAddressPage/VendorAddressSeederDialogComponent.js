...
import seederData from "./vendorAddress.seeder.json";
...
const VendorAddressSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("vendorAddress").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(mapState, mapDispatch)(VendorAddressSeederDialogComponent);
