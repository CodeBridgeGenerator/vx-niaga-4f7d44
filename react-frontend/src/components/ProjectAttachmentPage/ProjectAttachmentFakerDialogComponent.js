...
import projectAttachmentFakerFactory from "./projectAttachmentFakerFactory";
...
const ProjectAttachmentFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = projectAttachmentFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("projectAttachment").create(elem));
...
export default connect(mapState, mapDispatch)(ProjectAttachmentFakerDialogComponent);
