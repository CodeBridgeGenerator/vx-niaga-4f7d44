...
import seederData from "./projectAttachment.seeder.json";
...
const ProjectAttachmentSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("projectAttachment").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(mapState, mapDispatch)(ProjectAttachmentSeederDialogComponent);
