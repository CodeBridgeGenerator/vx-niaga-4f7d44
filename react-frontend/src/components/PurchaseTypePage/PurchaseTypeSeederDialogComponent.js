...
import seederData from "./purchaseType.seeder.json";
...
const PurchaseTypeSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("purchaseType").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(mapState, mapDispatch)(PurchaseTypeSeederDialogComponent);
