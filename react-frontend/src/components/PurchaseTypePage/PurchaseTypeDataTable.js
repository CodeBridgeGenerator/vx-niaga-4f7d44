...
const PurchaseTypeDataTable = ({ items, fields, onEditRow, onRowDelete, onRowClick, searchDialog, setSearchDialog,   showUpload, setShowUpload,
...
const p_numberTemplate3 = (rowData, { rowIndex }) => <p >{rowData.updatedAt}</p>
    ...
    const paginatorRight = DownloadCSV({ data : items, fileName : "purchaseType"});
...
<Column field="updatedAt" header="updated_at" body={p_numberTemplate3} filter={selectedFilterFields.includes("updatedAt")} hidden={selectedHideFields?.includes("updatedAt")}  sortable style={{ minWidth: "8rem" }} />
        ...
        <Dialog header="Upload PurchaseType Data" visible={showUpload} onHide={() => setShowUpload(false)}>
        <UploadService 
          user={user} 
          serviceName="purchaseType"            
          onUploadComplete={() => {
            setShowUpload(false); // Close the dialog after upload
          }}/>
      </Dialog>

      <Dialog header="Search PurchaseType" visible={searchDialog} onHide={() => setSearchDialog(false)}>
      Search
    </Dialog>
    <Dialog
        header="Filter Users"
...
export default PurchaseTypeDataTable;