...
import purchaseTypeFakerFactory from "./purchaseTypeFakerFactory";
...
const PurchaseTypeFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = purchaseTypeFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("purchaseType").create(elem));
...
export default connect(mapState, mapDispatch)(PurchaseTypeFakerDialogComponent);
