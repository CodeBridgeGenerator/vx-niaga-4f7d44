...
import customerTypesFakerFactory from "./customerTypesFakerFactory";
...
const CustomerTypesFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = customerTypesFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("customerTypes").create(elem));
...
export default connect(mapState, mapDispatch)(CustomerTypesFakerDialogComponent);
