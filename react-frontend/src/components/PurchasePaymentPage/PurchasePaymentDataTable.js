...
const PurchasePaymentDataTable = ({ items, fields, onEditRow, onRowDelete, onRowClick, searchDialog, setSearchDialog,   showUpload, setShowUpload,
...
const p_numberTemplate10 = (rowData, { rowIndex }) => <p >{rowData.updatedAt}</p>
    ...
    const paginatorRight = DownloadCSV({ data : items, fileName : "purchasePayment"});
...
<Column field="updatedAt" header="updated_at" body={p_numberTemplate10} filter={selectedFilterFields.includes("updatedAt")} hidden={selectedHideFields?.includes("updatedAt")}  sortable style={{ minWidth: "8rem" }} />
        ...
        <Dialog header="Upload PurchasePayment Data" visible={showUpload} onHide={() => setShowUpload(false)}>
        <UploadService 
          user={user} 
      ...
      <Dialog header="Search PurchasePayment" visible={searchDialog} onHide={() => setSearchDialog(false)}>
      Search
    </Dialog>
    <Dialog
        header="Filter Users"
...
export default PurchasePaymentDataTable;