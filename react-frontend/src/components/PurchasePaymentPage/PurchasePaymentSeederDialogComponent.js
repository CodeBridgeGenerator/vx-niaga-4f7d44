...
import seederData from "./purchasePayment.seeder.json";
...
const PurchasePaymentSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("purchasePayment").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(mapState, mapDispatch)(PurchasePaymentSeederDialogComponent);
