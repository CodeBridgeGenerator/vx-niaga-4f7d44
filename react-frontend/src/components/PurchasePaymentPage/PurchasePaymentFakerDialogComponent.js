...
import purchasePaymentFakerFactory from "./purchasePaymentFakerFactory";
...
const PurchasePaymentFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = purchasePaymentFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("purchasePayment").create(elem));
...
export default connect(mapState, mapDispatch)(PurchasePaymentFakerDialogComponent);
