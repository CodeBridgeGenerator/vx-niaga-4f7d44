...
import productBranchesFakerFactory from "./productBranchesFakerFactory";
...
const ProductBranchesFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = productBranchesFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("productBranches").create(elem));
...
export default connect(mapState, mapDispatch)(ProductBranchesFakerDialogComponent);
