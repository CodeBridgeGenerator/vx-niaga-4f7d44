...
const ProductBranchesDataTable = ({ items, fields, onEditRow, onRowDelete, onRowClick, searchDialog, setSearchDialog,   showUpload, setShowUpload,
...
const pTemplate3 = (rowData, { rowIndex }) => <p >{rowData.updatedAt}</p>
    ...
    const paginatorRight = DownloadCSV({ data : items, fileName : "productBranches"});
...
<Column field="updatedAt" header="updated_at" body={pTemplate3} filter={selectedFilterFields.includes("updatedAt")} hidden={selectedHideFields?.includes("updatedAt")}  sortable style={{ minWidth: "8rem" }} />
        ...
        <Dialog header="Upload ProductBranches Data" visible={showUpload} onHide={() => setShowUpload(false)}>
        <UploadService 
          user={user} 
      ...
      <Dialog header="Search ProductBranches" visible={searchDialog} onHide={() => setSearchDialog(false)}>
      Search
    </Dialog>
    <Dialog
        header="Filter Users"
...
export default ProductBranchesDataTable;