...
import calendarFakerFactory from "./calendarFakerFactory";
...
const CalendarFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = calendarFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("calendar").create(elem));
...
export default connect(mapState, mapDispatch)(CalendarFakerDialogComponent);
