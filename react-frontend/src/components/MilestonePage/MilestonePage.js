...
import MilestoneEditDialogComponent from "./MilestoneEditDialogComponent";
import MilestoneCreateDialogComponent from "./MilestoneCreateDialogComponent";
import MilestoneFakerDialogComponent from "./MilestoneFakerDialogComponent";
import MilestoneSeederDialogComponent from "./MilestoneSeederDialogComponent";
...
const MilestonePage = (props) => {
    ...
    const filename = "milestone.csv";

    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("milestone");
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("milestone").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "Milestone", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
        ...
        const promises = data.map((e) => client.service("milestone").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
        ...
        <div className="mt-5">
            <div className="grid">
                <div className="col-6 flex justify-content-start">
                    <h3 className="mb-0 ml-2">Milestone </h3>
                </div>
                <div className="col-6 flex justify-content-end">
                    <>
                    <Button label="add" loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="milestone-add-button"/>
            ...
            <div className="grid align-items-center">
                <div className="col-12" role="milestone-datatable">
                    <MilestoneDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>
            ...
            <MilestoneEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <MilestoneCreateDialogComponent entity={newRecord} show={showCreateDialog} onHide={() => setShowCreateDialog(false)} onCreateResult={onCreateResult} />
            <MilestoneFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <MilestoneSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(MilestonePage);
