...
import milestoneFakerFactory from "./milestoneFakerFactory";
...
const MilestoneFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = milestoneFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("milestone").create(elem));
...
export default connect(mapState, mapDispatch)(MilestoneFakerDialogComponent);
