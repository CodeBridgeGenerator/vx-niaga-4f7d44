...
const UnitCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            branchId: _entity?.branchId,name: _entity?.name,description: _entity?.description,createdAt: _entity?.createdAt,updatedAt: _entity?.updatedAt,
        ...
        const result = await client.service("unit").create(_data);
        props.onHide();
        props.alert({ type: "success", title: "Create info", message: "Info Unit created successfully" });
        props.onCreateResult(result);
        } catch (error) {
            console.log("error", error);
            setError(getSchemaValidationErrorsStrings(error) || "Failed to create");
            props.alert({ type: "error", title: "Create", message: "Failed to create in Unit" });
        }
        setLoading(false);
    };
    ...
    return (
        <Dialog header="Create Unit" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max" footer={renderFooter()} resizable={false}>
            <div className="grid p-fluid overflow-y-auto"
            style={{ maxWidth: "55vw" }} role="unit-create-dialog-component">
...
export default connect(mapState, mapDispatch)(UnitCreateDialogComponent);
