...
import unitFakerFactory from "./unitFakerFactory";
...
const UnitFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = unitFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("unit").create(elem));
...
export default connect(mapState, mapDispatch)(UnitFakerDialogComponent);
