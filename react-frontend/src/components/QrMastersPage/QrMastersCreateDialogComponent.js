...
const QrMastersCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            branchId: _entity?.branchId,modelType: _entity?.modelType,modelId: _entity?.modelId,qrPath: _entity?.qrPath,createdAt: _entity?.createdAt,updatedAt: _entity?.updatedAt,
        ...
        props.onHide();
        props.alert({ type: "success", title: "Create info", message: "Info Qr Masters created successfully" });
