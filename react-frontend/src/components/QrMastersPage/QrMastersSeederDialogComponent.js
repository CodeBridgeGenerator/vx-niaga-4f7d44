...
import seederData from "./qrMasters.seeder.json";
...
const QrMastersSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("qrMasters").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(mapState, mapDispatch)(QrMastersSeederDialogComponent);
