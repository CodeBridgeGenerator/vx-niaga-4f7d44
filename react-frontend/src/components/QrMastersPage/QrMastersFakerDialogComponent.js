...
import qrMastersFakerFactory from "./qrMastersFakerFactory";
...
const QrMastersFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = qrMastersFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("qrMasters").create(elem));
...
export default connect(mapState, mapDispatch)(QrMastersFakerDialogComponent);
