...
const QrMastersDataTable = ({ items, fields, onEditRow, onRowDelete, onRowClick, searchDialog, setSearchDialog,   showUpload, setShowUpload,
...
const pTemplate3 = (rowData, { rowIndex }) => <p >{rowData.qrPath}</p>
...
const p_numberTemplate5 = (rowData, { rowIndex }) => <p >{rowData.updatedAt}</p>
    ...
    const paginatorRight = DownloadCSV({ data : items, fileName : "qrMasters"});
...
<Column field="qrPath" header="qr_path" body={pTemplate3} filter={selectedFilterFields.includes("qrPath")} hidden={selectedHideFields?.includes("qrPath")}  sortable style={{ minWidth: "8rem" }} />
...
<Column field="updatedAt" header="updated_at" body={p_numberTemplate5} filter={selectedFilterFields.includes("updatedAt")} hidden={selectedHideFields?.includes("updatedAt")}  sortable style={{ minWidth: "8rem" }} />
        ...
        <Dialog header="Upload QrMasters Data" visible={showUpload} onHide={() => setShowUpload(false)}>
        <UploadService 
          user={user} 
          serviceName="qrMasters"            
          onUploadComplete={() => {
            setShowUpload(false); // Close the dialog after upload
          }}/>
      </Dialog>

      <Dialog header="Search QrMasters" visible={searchDialog} onHide={() => setSearchDialog(false)}>
      Search
    </Dialog>
    <Dialog
        header="Filter Users"
...
export default QrMastersDataTable;