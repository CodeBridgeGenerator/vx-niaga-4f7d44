...
const PaymentMethodsCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            branchId: _entity?.branchId,name: _entity?.name,accountId: _entity?.accountId,createdAt: _entity?.createdAt,updatedAt: _entity?.updatedAt,
    ...
    return (
        <Dialog header="Create Payment Methods" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max" footer={renderFooter()} resizable={false}>
