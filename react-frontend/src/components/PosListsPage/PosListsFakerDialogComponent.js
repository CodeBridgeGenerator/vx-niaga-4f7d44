...
import posListsFakerFactory from "./posListsFakerFactory";
...
const PosListsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = posListsFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("posLists").create(elem));
...
export default connect(mapState, mapDispatch)(PosListsFakerDialogComponent);
