...
const PosListsDataTable = ({ items, fields, onEditRow, onRowDelete, onRowClick, searchDialog, setSearchDialog,   showUpload, setShowUpload,
...
const p_numberTemplate1 = (rowData, { rowIndex }) => <p >{rowData.warehouseId}</p>
const pTemplate2 = (rowData, { rowIndex }) => <p >{rowData.name}</p>
const pTemplate3 = (rowData, { rowIndex }) => <p >{rowData.userIds}</p>
...
const p_numberTemplate6 = (rowData, { rowIndex }) => <p >{rowData.updatedAt}</p>
    ...
    const paginatorRight = DownloadCSV({ data : items, fileName : "posLists"});
...
<Column field="warehouseId" header="warehouse_id" body={p_numberTemplate1} filter={selectedFilterFields.includes("warehouseId")} hidden={selectedHideFields?.includes("warehouseId")}  sortable style={{ minWidth: "8rem" }} />
...
<Column field="userIds" header="user_ids" body={pTemplate3} filter={selectedFilterFields.includes("userIds")} hidden={selectedHideFields?.includes("userIds")}  sortable style={{ minWidth: "8rem" }} />
...
<Column field="updatedAt" header="updated_at" body={p_numberTemplate6} filter={selectedFilterFields.includes("updatedAt")} hidden={selectedHideFields?.includes("updatedAt")}  sortable style={{ minWidth: "8rem" }} />
        ...
        <Dialog header="Upload PosLists Data" visible={showUpload} onHide={() => setShowUpload(false)}>
        <UploadService 
          user={user} 
          serviceName="posLists"            
          onUploadComplete={() => {
            setShowUpload(false); // Close the dialog after upload
          }}/>
      </Dialog>

      <Dialog header="Search PosLists" visible={searchDialog} onHide={() => setSearchDialog(false)}>
      Search
    </Dialog>
    <Dialog
        header="Filter Users"
...
export default PosListsDataTable;