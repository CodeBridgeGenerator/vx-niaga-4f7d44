...
const WarehousesCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            branchId: _entity?.branchId,name: _entity?.name,address: _entity?.address,city: _entity?.city,zipcode: _entity?.zipcode,createdAt: _entity?.createdAt,updatedAt: _entity?.updatedAt,
        ...
        props.onHide();
        props.alert({ type: "success", title: "Create info", message: "Info Warehouses created successfully" });
