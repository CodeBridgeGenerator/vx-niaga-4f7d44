...
import warehousesFakerFactory from "./warehousesFakerFactory";
...
const WarehousesFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = warehousesFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("warehouses").create(elem));
...
export default connect(mapState, mapDispatch)(WarehousesFakerDialogComponent);
