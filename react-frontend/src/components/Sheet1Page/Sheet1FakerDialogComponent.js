...
import sheet1FakerFactory from "./sheet1FakerFactory";
...
const Sheet1FakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = sheet1FakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("sheet1").create(elem));
...
export default connect(mapState, mapDispatch)(Sheet1FakerDialogComponent);
