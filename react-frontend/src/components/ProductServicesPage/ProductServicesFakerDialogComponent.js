...
import productServicesFakerFactory from "./productServicesFakerFactory";
...
const ProductServicesFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = productServicesFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("productServices").create(elem));
...
export default connect(mapState, mapDispatch)(ProductServicesFakerDialogComponent);
