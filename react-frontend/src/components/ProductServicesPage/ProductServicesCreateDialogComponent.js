...
const ProductServicesCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            branchId: _entity?.branchId,sku: _entity?.sku,name: _entity?.name,type: _entity?.type,categoryId: _entity?.categoryId,unitId: _entity?.unitId,stockQuantity: _entity?.stockQuantity,description: _entity?.description,createdAt: _entity?.createdAt,updatedAt: _entity?.updatedAt,
        ...
        props.onHide();
        props.alert({ type: "success", title: "Create info", message: "Info Product Services created successfully" });
...
<div className="col-12 md:col-6 field mt-5">
            <span className="align-items-center">
                <label htmlFor="unitId">unit_id:</label>
                <InputNumber id="unitId" className="w-full mb-3 p-inputtext-sm" value={_entity?.unitId} onChange={(e) => setValByKey("unitId", e.value)}  />
            </span>
            <small className="p-error">
